...
static uint32_t series_7idcodes[NUM_7SERIES_IDCODES] = {
...
static uint32_t series_ultra_idcodes[NUM_ULTRA_SERIES_IDCODES] = {
    0x3824093, 0x3823093, 0x3822093, 0x3919093, 0x380F093, 0x3844093,
    0x390D093, 0x3939093, 0x3843093, 0x3842093, 0x392D093, 0x3933093,
    0x3931093, 0x396D093
};

static uint32_t series_ultra_plus_idcodes[NUM_ULTRA_PLUS_SERIES_IDCODES] = {
...
static void StubStatusHandler(void *CallBackRef, uint32_t StatusEvent,
                  uint32_t ByteCount);
static uint32_t FindDeviceType(uint32_t IdCode);
...
#if XPAR_HWICAP_0_ICAP_DWIDTH == 8
    uint8_t Data[NumWords * 4];
#elif XPAR_HWICAP_0_ICAP_DWIDTH == 16
    uint16_t Data[NumWords * 2];
#else
    uint32_t *Data = FrameBuffer;
#endif
    uint32_t RdFifoOccupancy = 0;
...
*
******************************************************************************/
static void StubStatusHandler(void *CallBackRef, uint32_t StatusEvent, uint32_t ByteCount)
{
    (void) CallBackRef;
    (void) StatusEvent;
    (void) ByteCount;
