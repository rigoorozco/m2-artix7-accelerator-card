...
localparam STATE_WR_CMD = 4'd1;
localparam STATE_RD_STS = 4'd2;

localparam ADDR_USR_ENABLE      = 8'h00;
localparam ADDR_USR_AUTO_ENABLE = 8'h01;
localparam ADDR_USR_SCRATCH     = 8'h02;
localparam ADDR_USR_RESET       = 8'h03;
localparam ADDR_CMD_TAG         = 8'h04;
localparam ADDR_CMD_SADDR_HIGH  = 8'h05;
localparam ADDR_CMD_SADDR_LOW   = 8'h06;
localparam ADDR_CMD_DDR         = 8'h07;
localparam ADDR_CMD_EOF         = 8'h08;
localparam ADDR_CMD_DSA         = 8'h09;
...
always @(negedge up_rstn or posedge up_clk) begin
    if (up_rstn == 0) begin
        ...
        usr_enable      <= DEFAULT_ENABLE;
        usr_auto_enable <= 0;
        usr_scratch     <= 32'hBEEFCAFE;
        usr_reset       <= 0;

        cmd_tag         <= 'h0;
        cmd_saddr       <= DEFAULT_ADDR;
        cmd_ddr         <= 0;
        cmd_eof         <= 0;
        cmd_dsa         <= 'h0;
        cmd_type        <= 0;
        cmd_btt         <= DEFAULT_SIZE;
        ...
        if (up_wreq) begin
            case(up_waddr[7:0])
                ...
                ADDR_USR_RESET:       usr_reset        <= 1;
                ADDR_CMD_TAG:         cmd_tag          <= up_wdata[3:0];
                ADDR_CMD_SADDR_HIGH:  cmd_saddr[63:32] <= up_wdata;
                ADDR_CMD_SADDR_LOW:   cmd_saddr[31: 0] <= up_wdata;
                ADDR_CMD_DDR:         cmd_ddr          <= up_wdata[0];
                ADDR_CMD_EOF:         cmd_eof          <= up_wdata[0];
                ADDR_CMD_DSA:         cmd_dsa          <= up_wdata[5:0];
                ADDR_CMD_TYPE:        cmd_type         <= up_wdata[0];
                ADDR_CMD_BTT:         cmd_btt          <= up_wdata[22:0];
        ...
        if (up_rreq) begin
            case(up_raddr[7:0])
                ...
                ADDR_USR_RESET:       up_rdata   <= 32'hDEADBEEF; // WRITE-ONLY
                ADDR_CMD_TAG:         up_rdata   <= {18'd0, cmd_tag};
                ADDR_CMD_SADDR_HIGH:  up_rdata   <= cmd_saddr[63:32];
                ADDR_CMD_SADDR_LOW:   up_rdata   <= cmd_saddr[31: 0];
                ADDR_CMD_DDR:         up_rdata   <= {31'd0, cmd_ddr};
                ADDR_CMD_EOF:         up_rdata   <= {31'd0, cmd_eof};
